...
$template['css']=array('style/public.css','style/show.css','style/style.css');
?>
    <!DOCTYPE html>
    <html lang="zh-CN">
    <head>
...
<!--        <a href="index.php">首页</a> &gt; <a href="list_father.php?id=--><?php //echo $data_father['id']?><!--">--><?php //echo $data_father['module_name']?><!--</a> &gt; <a href="list_son.php?id=--><?php //echo $data_son['id']?><!--">--><?php //echo $data_son['module_name']?><!--</a> &gt; --><?php //echo $data_content['title']?>
    ...
    <div id="main" class="auto">
        <div class="wrap1">
            <div class="pages">
                <?php
                $query="select count(*) from  sfk_reply_message where content_id={$_GET['id']}";
                $count_reply=num($link, $query);
                $page_size=5;
...
<!--            <a class="btn reply" href="reply.php?id=--><?php //echo $_GET['id']?><!--" target="_blank"></a>-->
        ...
        <?php
        $query="select sm.name,sr.member_id,sr.quote_id,sm.photo,sr.time,sr.id,sr.content from sfk_reply_message sr,sfk_member sm where sr.member_id=sm.id and sr.content_id={$_GET['id']} order by id asc {$page['limit']}";
        ...
        while ($data_reply=mysqli_fetch_assoc($result_reply)){
            ...
            <div class="wrapContent">
                ...
                <div class="right">
                    <div class="pubdate">
                        <span class="date">评论时间：<?php echo $data_reply['time']?></span>
                    ...
                    <div class="content">
                        ...
                        if($data_reply['quote_id']){
                            $query="select count(*) from sfk_reply_message where content_id={$_GET['id']} and id<={$data_reply['quote_id']}";
                            $floor=num($link,$query);
                            $query="select sfk_reply_message.content,sfk_member.name from sfk_reply_message,sfk_member where sfk_reply_message.id={$data_reply['quote_id']} and sfk_reply_message.content_id={$_GET['id']} and sfk_reply_message.member_id=sfk_member.id";
                            $result_quote=execute($link,$query);
                            $data_quote=mysqli_fetch_assoc($result_quote);
                            ?>
                            <div class="quote">
                                <h2>回复 <?php echo $floor?>楼 <?php echo $data_quote['name']?> 发表的: </h2>
...
<!--            <a class="btn reply" href="reply_message.php?id=--><?php //echo $_GET['id']?><!--" target="_blank"></a>-->
            <div style="clear:both;"></div>
        </div>
    </div>
</body>
